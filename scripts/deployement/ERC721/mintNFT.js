...
async function mint(){
    ...
    const CollectionAddress = addressFile["ERC721_contracts"]["MyCollection"]["CollectionAddress"];
    console.log("Collection Address: ", CollectionAddress);

    //
    const imageName = mintingFile["mintingData"]["name"];
    const Receiver= mintingFile["mintingData"]["Receiver"];
  ...
  if (items.length === 0  ) {
    ...
    throw new Error("More than one item found to mint, the image Name is duplicated. Please name the images differently");
  } else {
  ...
  metadataFilePath = "./ressources/ERC721/metadata/"+ path.basename(items[0], path.extname(items[0])) + ".json"
  // path.join(
  //   __dirname,
...
try{
  ...
  const collectionContract= await ethers.getContractAt("MyCollection", CollectionAddress);

  const mint = await collectionContract.mint(Receiver, URI,{
